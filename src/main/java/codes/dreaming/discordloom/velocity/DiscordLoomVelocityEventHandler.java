...
public class DiscordLoomVelocityEventHandler {
    ...
    private void handleNicknameSync(User user) throws SerializationException {

        LOGGER.info("Syncing nicknames");

        if (config.getNicknameSyncEnabledGuilds().isEmpty()) {
            return;
        }

        if (PermissionHelper.hasPermission(user, MOD_ID + ".bypassNicknameSync")) {
            return;
        }
        ...
        if (discordIdNode.isEmpty()) {
            logger.debug("Player {} does not have a Discord account linked", user.getUsername());
            return;
        }
        ...
        if (nicknameNode.isEmpty()) {
            logger.debug("Player {} does not have an in-game nickname set with key '{}'", user.getUsername(), nicknameMetadataKey);
            return;
        }
        ...
        config.getNicknameSyncEnabledGuilds().forEach(guildId -> {
            ...
            if (minecraftNickname.equals(currentNickname)) {
                logger.debug("Discord nickname for {} ({}) is already set to '{}' in guild {}",
                        user.getUsername(), discordUserId, minecraftNickname, guildId);
                return;
            }

            guild.modifyNickname(member, minecraftNickname)
                    .reason("Minecraft nickname sync for " + user.getUsername())
                    .queue(
                            ...
                            error -> logger.error("Failed to update Discord nickname for {} ({}) in guild {}: {}",
                                    user.getUsername(), discordUserId, guildId, error.getMessage())
                    );
        });
    }
